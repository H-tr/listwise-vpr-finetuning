...
from models.cct import cct_14_7x2_384
from models.aggregations import Flatten
from models.normalization import L2Norm
import models.aggregations as aggregation
...
def get_aggregation(args):
    if args.aggregation == "gem":
        ...
        return aggregation.CRN(
            clusters_num=args.netvlad_clusters, dim=args.features_dim
        )
...
def get_pretrained_model(args):
    ...
    if not os.path.exists(file_path):
        gdd.download_file_from_google_drive(
            file_id=PRETRAINED_MODELS[model_name], dest_path=file_path
        )
...
def get_backbone(args):
    ...
    if args.backbone.startswith("resnet"):
        ...
        if args.backbone.startswith("cct384"):
            backbone = cct_14_7x2_384(
                pretrained=True, progress=True, aggregation=args.aggregation
            )
        if args.trunc_te:
            logging.debug(f"Truncate CCT at transformers encoder {args.trunc_te}")
            backbone.classifier.blocks = torch.nn.ModuleList(
                backbone.classifier.blocks[: args.trunc_te].children()
            )
        if args.freeze_te:
            logging.debug(
        ...
        assert args.resize[0] in [
            224,
            384,
        ], f"Image size for ViT must be either 224 or 384, but it's {args.resize[0]}"
        ...
        if args.freeze_te:
            logging.debug(
                f"Freeze all the layers up to tranformer encoder {args.freeze_te+1}"
            )
            for p in backbone.parameters():
                p.requires_grad = False
