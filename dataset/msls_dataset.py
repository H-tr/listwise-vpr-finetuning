...
class MSLSDataset(Dataset):
        ...
        for city in self.cities:
            ...
            if self.mode in ["train", "val"]:
                ...
                dbSeqKeys, dbSeqIdxs = self.arange_as_seq(
                    dbData, join(root_dir, subdir, city, "database"), seq_length_db
                )

                # filter based on subtasks
                if self.mode in ["val"]:
                    qIdx = pd.read_csv(
                        join(root_dir, subdir, city, "query", "subtask_index.csv"),
                        index_col=0,
                    )
                    dbIdx = pd.read_csv(
                        join(root_dir, subdir, city, "database", "subtask_index.csv"),
                        index_col=0,
                    )
                ...
                frameIdx2seqIdx = lambda frameIdx, seqIdxs: np.where(
                    seqIdxs == frameIdx
                )[0][1]
                frameIdx2uniqFrameIdx = lambda frameIdx, uniqFrameIdx: np.where(
                    np.in1d(uniqFrameIdx, frameIdx)
                )[0]
                uniqFrameIdx2seqIdx = lambda frameIdxs, seqIdxs: np.where(
                    np.in1d(seqIdxs, frameIdxs).reshape(seqIdxs.shape)
                )[0]
                ...
                night, sideways, index = (
                    qData["night"].values,
                    (qData["view_direction"] == "Sideways").values,
                    qData.index,
                )
                for q_seq_idx in range(len(qSeqKeys)):
                    ...
                    p_uniq_frame_idxs = np.unique(
                        [p for pos in I[q_uniq_frame_idx] for p in pos]
                    )
            ...
            # when GPS / UTM / pano info is not available
                ...
                dbSeqKeys, dbSeqIdxs = self.arange_as_seq(
                    dbIdx, join(root_dir, subdir, city, "database"), seq_length_db
                )
    ...
    def __calcSamplingWeights__(self):
        ...
        print(
            "#Sideways [{}/{}]; #Night; [{}/{}]".format(
                len(self.sideways), N, len(self.night), N
            )
        )
        print("Forward and Day weighted with {:.4f}".format(1))
        if len(self.night) != 0:
            print(
                "Forward and Night weighted with {:.4f}".format(1 + N / len(self.night))
            )
        if len(self.sideways) != 0:
            print(
                "Sideways and Day weighted with {:.4f}".format(
                    1 + N / len(self.sideways)
                )
            )
        if len(self.sideways) != 0 and len(self.night) != 0:
            print(
                "Sideways and Night weighted with {:.4f}".format(
    ...
    def arange_as_seq(self, data, path, seq_length):
        ...
        for idx in data.index:
            ...
            if (
                len(np.unique(seq["sequence_key"])) == 1
    ...
    def update_subcache(self, net=None):
        ...
        qloader = torch.utils.data.DataLoader(
            ImagesFromList(self.qImages[qidxs], transform=self.transform), **opt
        )
        ploader = torch.utils.data.DataLoader(
            ImagesFromList(self.dbImages[pidxs], transform=self.transform), **opt
        )
        nloader = torch.utils.data.DataLoader(
            ImagesFromList(self.dbImages[nidxs], transform=self.transform), **opt
        )
        ...
        with torch.no_grad():
            ...
            for i, batch in tqdm(
                enumerate(ploader), desc="compute positive descriptors"
            ):
                X, y = batch
                pvecs[i * bs : (i + 1) * bs, :] = net(X.to(self.device)).data
            for i, batch in tqdm(
                enumerate(nloader), desc="compute negative descriptors"
            ):
                X, y = batch
    ...
    def __getitem__(self, idx):
        ...
        output = [
            torch.stack(
                [self.transform(Image.open(im)) for im in self.qImages[qidx].split(",")]
            )
        ]
        output.append(
            torch.stack(
                [
                    self.transform(Image.open(im))
                    for im in self.dbImages[pidx].split(",")
                ]
            )
        )
        output.extend(
            [
                torch.stack(
                    [
                        self.transform(Image.open(im))
                        for im in self.dbImages[idx].split(",")
                    ]
                )
                for idx in nidx
            ]
